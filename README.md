...
## Legacy DB Uyumluluğu (Unknown column status)
- Eğer eski tablolar nedeniyle `Unknown column status` hatası oluşursa sistem artık otomatik kolon patch uygular.
- Hem `setup/install.php` hem de `app/bootstrap.php` eksik kritik kolonları (users.role/users.status/courses.status vb.) güvenli şekilde eklemeyi dener.
